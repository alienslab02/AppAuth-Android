 ...
 */
public class LogoutManagementActivity extends Activity {

    @VisibleForTesting
    static final String KEY_LOGOUT_INTENT = "logoutIntent";

    @VisibleForTesting
    static final String KEY_LOGOUT_REQUEST = "logoutRequest";
    ...
    @VisibleForTesting
    static final String KEY_LOGOUT_STARTED = "logoutStarted";
     ...
     * @param request the logout request which is to be sent.
     * @param logoutIntent the intent to be used to get logout the user.
    ...
    public static Intent createStartIntent(
        ...
        intent.putExtra(KEY_LOGOUT_INTENT, logoutIntent);
        intent.putExtra(KEY_LOGOUT_REQUEST, request.jsonSerializeString());
     ...
     * the original LogoutManagementActivity that was created at the start of the flow.
     * @param context the package context for the app.
    ...
    private static Intent createBaseIntent(Context context) {
        return new Intent(context, LogoutManagementActivity.class);
    }
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        outState.putBoolean(KEY_LOGOUT_STARTED, mLogoutStarted);
        outState.putParcelable(KEY_LOGOUT_INTENT, mLogoutIntent);
        outState.putString(KEY_LOGOUT_REQUEST, mLogoutRequest.jsonSerializeString());
    ...
    private void handleLogoutComplete() {
        ...
        if (responseData == null) {
            Logger.error("Failed to extract Logout response from redirect");
            return;
        }
        responseData.setData(responseUri);

        Logger.debug("Logout complete - invoking completion intent");
        try {
    ...
    private void handleLogoutCanceled() {
        Logger.debug("Logout flow canceled by user");
    ...
    private void extractState(Bundle state) {
        ...
        mLogoutIntent = state.getParcelable(KEY_LOGOUT_INTENT);
        mLogoutStarted = state.getBoolean(KEY_LOGOUT_STARTED, false);
        try {
            String logoutRequestJson = state.getString(KEY_LOGOUT_REQUEST, null);
            mLogoutRequest = logoutRequestJson != null
                    ? LogoutRequest.jsonDeserialize(logoutRequestJson)
                    : null;
...
//        } else {
//            AuthorizationResponse response = new AuthorizationResponse.Builder(mLogoutRequest)
//                    .fromUri(responseUri)
//                    .build();
//
//            if (mLogoutRequest.state == null && response.state != null
//                    || (mLogoutRequest.state != null && !mLogoutRequest.state.equals(response.state))) {
//                Logger.warn("State returned in authorization response (%s) does not match state "
//                        + "from request (%s) - discarding response",
//                        response.state,
//                        mLogoutRequest.state);
//
